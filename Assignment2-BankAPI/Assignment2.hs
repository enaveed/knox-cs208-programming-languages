...
deposit conn acc amt = do
  -- Inserting one (account, amount) event row
  execute conn depositQuery (acc, amt)
  pure ()
...
balance conn acc = do
  ...
  let name = T.unpack acc
      total = sum [amt | (acct, amt) <- rows, acct == name]
  pure total
...
perform _  Nothing             = pure (T.pack "ERROR")
perform db (Just (Deposit a n)) = do
  deposit db a n
  pure (T.pack "OK")

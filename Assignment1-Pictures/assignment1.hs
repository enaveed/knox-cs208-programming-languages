...
dotAndLine =  Picture f
  where
    f (Coord x y)
      | x == 3 && y == 4 = white   -- dot
      | y == 8           = pink    -- the whole row where y is 8 becomes pink
      | otherwise        = black   -- everything else stays black
------------------------------------------------------------------------------
...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) =
  Color ((r1 + r2) `div` 2)   -- red average
        ((g1 + g2) `div` 2)   -- green average
        ((b1 + b2) `div` 2)   -- blue average

-- note: Picture is just (Coord -> Color), so i’m literally composing functions.
combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine mix (Picture f1) (Picture f2) =
  Picture (\coord -> mix (f1 coord) (f2 coord))
...
-- a point (x,y) is inside the rectangle iff:
-- x is between x0 and (x0 + w - 1)
-- y is between y0 and (y0 + h - 1)
-- note to myself: use < (strict) with x0+w and y0+h to avoid off-by-one bugs.

rectangle :: Int -> Int -> Int -> Int -> Shape
rectangle x0 y0 w h = 
   Shape (\(Coord x y) ->
    x >= x0 && x < x0 + w &&   -- width range: x0 .. x0+w-1
    y >= y0 && y < y0 + h)     -- height range: y0 .. y0+h-1
------------------------------------------------------------------------------
...
-- union: inside if it's inside s1 OR inside s2
union :: Shape -> Shape -> Shape
union (Shape a) (Shape b) =
  Shape (\coord -> a coord || b coord)  -- either one says True -> inside

-- cut: keep points from s1 but remove anything covered by s2
-- (read it as: s1 minus s2)
cut :: Shape -> Shape -> Shape
cut (Shape a) (Shape b) =
  Shape (\coord -> a coord && not (b coord))  -- in a AND not in b
------------------------------------------------------------------------------
...
-- reminder: Picture is just (Coord -> Color), Shape is (Coord -> Bool).
-- here we draw a solid color ON TOP of an existing picture, but only
-- where the shape says True. outside the shape, keep the base pixel.
paintSolid :: Color -> Shape -> Picture -> Picture
paintSolid color (Shape inside) (Picture baseF) =
  Picture (\coord ->
    if inside coord
       then color       -- inside the shape -> overwrite with my solid color
       else baseF coord -- outside -> leave the original pixel alone
  )
------------------------------------------------------------------------------
...
-- the idea: same as paintSolid but instead of a fixed color, using a pattern Picture.
-- for each (x,y):
--   - if the shape says True -> taking the pattern’s color at that (x,y)
--   - otherwise keeping whatever the base picture had there
paint :: Picture -> Shape -> Picture -> Picture
paint (Picture patF) (Shape inside) (Picture baseF) =
  Picture (\coord ->
    if inside coord
       then patF coord     -- inside shape -> drawn pattern here (pattern depends on x,y)
       else baseF coord    -- outside shape -> leaving base pixel untouched
  )
------------------------------------------------------------------------------
...
-- Zoom: scale up by a factor z (same as the zoom function above).
-- note to self: z must be > 0 (or div-by-zero). I’ll assume valid input.
...
-- idea: for each (x,y), average the color at (x,y) with its 4 neighbors:
--       left (x-1,y), right (x+1,y), up (x,y-1), down (x,y+1).
-- note: integer division (rounding down) is what we want here.
...
instance Transform Blur where
  apply Blur (Picture f) =
    ...
    where
      avgAt (Coord x y) =
        let c0 = f (Coord  x     y   ) -- center
            cL = f (Coord (x-1)  y   ) -- left
            cR = f (Coord (x+1)  y   ) -- right
            cU = f (Coord  x    (y-1)) -- up
            cD = f (Coord  x    (y+1)) -- down

            -- tiny helpers so the math reads clearly
            add (Color r1 g1 b1) (Color r2 g2 b2) =
              Color (r1+r2) (g1+g2) (b1+b2)
            div5 (Color r g b) =
              Color (r `div` 5) (g `div` 5) (b `div` 5)
        in div5 (c0 `add` cL `add` cR `add` cU `add` cD)
        -- (red with red, green with green, blue with blue
------------------------------------------------------------------------------
...
-- idea: apply Blur repeatedly n times.
-- if n <= 0, do nothing (return the original picture).

instance Transform BlurMany where
  apply (BlurMany n) pic = go (max 0 n) pic
    where
      go 0 p = p                      -- base case: done blurring
      go k p = go (k - 1) (apply Blur p)  -- blur once, decrement, repeat
      -- k :: int
      -- p :: Picture
      -- n = requested blur count
      -- k = remaining blurs during the loop
      -- p = the picture at this moment in the loop (updated each time)
